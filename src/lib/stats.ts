 ...
 * finals night on record is a scoreline and nothing else. Callers either fall
 * back to something result-only (`predict.ts`) or drop the match entirely
 * (`formMatchups`); nobody pretends a blank sheet means nobody did anything.
 *
 * Two consumers, one definition. `predict.ts` compares it between the two
 * sides of one match, where era and match length cancel out on their own. The
 * form match-ups below compare it *across* matches, which they can't — see
 * `leaguePar`.
 */
...
// ---------------------------------------------------------------------------
// Form match-ups (the "best" / "worst form against" tiles)
// ---------------------------------------------------------------------------

/**
 * The league's mean contribution per set, season by season.
 *
 * The head-to-head tiles above ask "who do you beat?". These ones ask "who do
 * you play *well* against?", which means comparing one player's ledger across
 * matches years apart — and that comparison is not safe raw, because the
 * ledger's own definition changed underneath it. `Errors Forced` doesn't exist
 * in Season 1 (0 of 164 rows), so an S1 night scores about 1.9/set lower than
 * a Season 4 night for reasons that have nothing to do with who was across the
 * net. Measured league-wide: S1 −3.93, S2 −2.54, S3 −2.02, S4 −1.61.
 *
 * Subtracting the season's own par turns every match into "how far above or
 * below the league that night was", which is the only unit that can be
 * averaged over an eleven-year career. It also absorbs the general drift in
 * the table above, which continues past the S1 column gap.
 *
 * Memoised on the row array's identity — `loadStatRows` caches, and
 * `site-data.ts` hands the same array to all thirty player pages, so this runs
 * once per build and not once per player.
 */
const leagueParCache = new WeakMap<StatRow[], Map<number, number>>();

export function leaguePar(rows: StatRow[]): Map<number, number> {
  const cached = leagueParCache.get(rows);
  if (cached) return cached;

  const acc = new Map<number, { total: number; sets: number }>();
  for (const r of rows) {
    if (r.scheduled || r.isSingles) continue;
    const c = contribution(r);
    if (c === null) continue;
    const e = acc.get(r.season) ?? { total: 0, sets: 0 };
    e.total += c;
    e.sets += r.sets;
    acc.set(r.season, e);
  }
  ...
  const par = new Map(
    [...acc].map(([season, e]) => [season, e.sets ? e.total / e.sets : 0])
  );
  leagueParCache.set(rows, par);
  return par;
}

/**
 * The stats a form tile is allowed to blame. Deliberately the five that make up
 * `contribution`, so the justification line always explains the number above
 * it rather than introducing a new one. `bad` flips the copy: a *drop* in
 * unforced errors is the most common reason a player's form tile fires at all,
 * and "3.4 fewer than usual" has to read as the good news it is.
 */
const FORM_STATS = [
  { stat: 'winners', label: 'winners', bad: false },
  { stat: 'unforcedErrors', label: 'unforced errors', bad: true },
  { stat: 'aces', label: 'aces', bad: false },
  { stat: 'doubleFaults', label: 'double faults', bad: true },
  { stat: 'errorsForced', label: 'errors forced', bad: false },
] as const satisfies readonly { stat: CountingStat; label: string; bad: boolean }[];

/** One statted meeting, with what the player's ledger did that night. */
export interface FormMeeting {
  ...
  fillIn: boolean;
  opponentFillIn: boolean;
  /** Season-centred contribution per set for this match. */
  perSet: number;
}

/** The one stat that moved most between the match-up and the player's norm. */
export interface FormReason {
  stat: CountingStat;
  /** Lower-case display label, e.g. "unforced errors". */
  label: string;
  /** True when a bigger number is a worse thing. */
  bad: boolean;
  /** Per-set rate against this opponent. */
  value: number;
  /** Per-set rate over every other statted match. */
  baseline: number;
  /** `value - baseline`. Signed; the copy reads the sign against `bad`. */
  delta: number;
}

/** How a player's form compares against one opponent, versus their own norm. */
export interface FormMatchup {
  opponent: string;
  slug: string;
  /** STATTED meetings. A scoreline-only final is not evidence about form. */
  meetings: number;
  /** Season-centred contribution per set against this opponent. */
  perSet: number;
  /** The same, over every statted match NOT against them. */
  baseline: number;
  /** `perSet - baseline`. The number the tiles are ranked on. */
  delta: number;
  /** Why, or null when no stat has coverage on both sides of the split. */
  reason: FormReason | null;
  /** Every statted meeting, oldest first. */
  games: FormMeeting[];
}

export interface FormSplit {
  /** Statted meetings needed to qualify — chosen adaptively, as for the H2H. */
  minMeetings: number;
  best: FormMatchup;
  worst: FormMatchup;
}

/** A player's statted match plus the opponents it counts against. */
interface FormMatch {
  row: StatRow;
  opponents: StatRow[];
  /** Contribution less the season's par for the sets played. */
  centred: number;
}

/** Set-weighted per-set rate for one stat, or null if nothing was recorded. */
function statRate(matches: FormMatch[], stat: CountingStat): number | null {
  let total = 0,
    sets = 0;
  for (const m of matches) {
    const v = m.row[stat];
  ...
  return sets ? total / sets : null;
}

/**
 * The opponents a player plays best and worst against — not by result, but by
 * their own stat ledger.
 *
 * The metric is `contribution` per set, centred on the season's league par
 * (see `leaguePar`), set-weighted the way every other rate on the site is:
 * `sum(centred) / sum(sets)`, so a three-set final counts for more court time
 * than a one-set Tuesday. The number on the tile is that average against one
 * opponent, less the same average over every *other* statted match — the
 * opponent is kept out of their own baseline, or the effect being reported
 * would be diluted by the very matches producing it.
 *
 * Three things this does differently to `bestWorstOpponent`:
 *
 *  - **It counts statted meetings only.** Three of the four finals brackets are
 *    scorelines with no player stats, and a match with no ledger says nothing
 *    about form. It doesn't join the tally, the baseline, or the qualifying
 *    count. That bites: some players have eighteen opponents and three with two
 *    statted meetings.
 *  - **The baseline is per opponent**, not one number per player, because each
 *    one excludes a different set of matches. Two tiles on the same page can
 *    quote slightly different norms. That's correct.
 *  - **No shrinkage.** Raw argmax, deliberately — shrinking toward the mean
 *    moved only three of fifty picks on the real CSV, which isn't worth the
 *    number on the tile no longer being the number in the popover. The cost is
 *    real and the fine print owns it: about a quarter of the picks rest on two
 *    matches, they're systematically the most extreme ones, and a tile can flip
 *    on a single new meeting. Don't add shrinkage back without asking.
 *
 * Fill-ins count on both sides, as they do for the head-to-head. Votes are
 * never read — which is what makes this safe on a sealed season.
 */
export function formMatchups(
  player: string,
  rows: StatRow[] = loadStatRows()
): FormSplit | null {
  const sides = fixtureSides(rows);
  const par = leaguePar(rows);

  // Every statted match the player has played, with the line-up across the net.
  ...
  for (const r of rows) {
    ...
    matches.push({ row: r, opponents: across, centred: c - (par.get(r.season) ?? 0) * r.sets });
  }

  const byOpponent = new Map<string, { slug: string; matches: FormMatch[] }>();
  for (const m of matches) {
    for (const o of m.opponents) {
      const e =
        byOpponent.get(o.player) ??
        byOpponent.set(o.player, { slug: o.slug, matches: [] }).get(o.player)!;
      // One match, one entry: `met.length` is the meeting count and `metSets`
      // the denominator, so a duplicated opposing row would double-count both.
      if (!e.matches.includes(m)) e.matches.push(m);
    }
  }

  const totalCentred = matches.reduce((n, m) => n + m.centred, 0);
  const totalSets = matches.reduce((n, m) => n + m.row.sets, 0);

  const candidates: FormMatchup[] = [];
  for (const [opponent, { slug, matches: met }] of byOpponent) {
    const metSets = met.reduce((n, m) => n + m.row.sets, 0);
    const restSets = totalSets - metSets;
    // Nothing to compare against: this opponent is the player's whole career.
    if (!metSets || !restSets) continue;

    const rest = matches.filter((m) => !met.includes(m));
    const metCentred = met.reduce((n, m) => n + m.centred, 0);
    const perSet = metCentred / metSets;
    const baseline = (totalCentred - metCentred) / restSets;

    // The one stat that moved most. A stat with no coverage on either side of
    // the split is skipped rather than read as zero — that's what stops an
    // S1-only match-up "explaining" itself with errors forced.
    let reason: FormReason | null = null;
    for (const { stat, label, bad } of FORM_STATS) {
      const value = statRate(met, stat);
      const base = statRate(rest, stat);
      if (value === null || base === null) continue;
      const delta = value - base;
      if (reason && Math.abs(delta) <= Math.abs(reason.delta)) continue;
      reason = { stat, label, bad, value, baseline: base, delta };
    }
    ...
    candidates.push({
      ...
      perSet,
      baseline,
      delta: perSet - baseline,
      reason,
      games: met
        .map((m) => ({
          season: m.row.season,
          round: m.row.round,
          roundLabel: m.row.roundLabel,
          stage: m.row.stage,
          isFinals: m.row.isFinals,
          score: m.row.score,
          sets: m.row.sets,
          team: m.row.team,
          opponentTeam: m.row.opponent,
          win: m.row.win,
          fillIn: m.row.isFillIn,
          opponentFillIn: m.opponents.some(
            (o) => o.player === opponent && o.isFillIn
          ),
          perSet: m.centred / m.row.sets,
        }))
        .sort((a, b) => a.season - b.season || a.round - b.round),
    });
  }

  // Same adaptive bar as the head-to-head tiles: the highest of the two that
  // still leaves two opponents to compare, and nothing at all below the floor.
  const floor = SITE.h2hMinMeetings;
  let minMeetings = floor;
  for (let n = SITE.h2hPreferredMeetings; n >= floor; n--) {
    if (candidates.filter((c) => c.meetings >= n).length >= 2) {
      minMeetings = n;
      break;
    }
  }

  const qualified = candidates.filter((c) => c.meetings >= minMeetings);
  if (qualified.length < 2) return null;

  const bestForm = (a: FormMatchup, b: FormMatchup) =>
    b.delta - a.delta || b.meetings - a.meetings || a.opponent.localeCompare(b.opponent);
  const worstForm = (a: FormMatchup, b: FormMatchup) =>
    a.delta - b.delta || b.meetings - a.meetings || a.opponent.localeCompare(b.opponent);

  return {
    minMeetings,
    best: [...qualified].sort(bestForm)[0],
    worst: [...qualified].sort(worstForm)[0],
  };
}
